...
- **0.31.2 crabd (2026-09-11)** - the lanes still did not show, and the session said it had
  finished while it was working. One cause each. The running count was an estimate - recently
  touched files minus a count of recent stop hooks - and on a dispatcher whose lanes turn over every
  thirty seconds that estimate sits at zero, which emptied the list and the badge alike. Claude Code
  has been firing `SubagentStart` all along; crabd subscribed to the exit and not the entry, so it
  now counts the balance exactly. Separately, a session whose turn ended while its subagents kept
  working stayed "finished": a subagent returning into the parent's loop now reopens it.
